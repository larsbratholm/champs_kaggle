...
def train(train_loader, model, optimizer, epoch, bond_types, scheduler):
    ...
    for step, (atom0, atom1, typ, xyz0, xyz1, mu0, mu1, mask, target, weight, length) in enumerate(train_loader):
        ...
        mae_loss_1 = torch.abs((pred5-target)*(target < 1e08).float() ).sum(-1).view(-1) * weight.view(-1)
        mae_loss_1 = mae_loss_1[mask.view(-1)]

        mae_loss_2 = torch.abs((pred1.view(-1) - target[:, :, 0].view(-1))
        mae_loss_2 = mae_loss_2[mask.view(-1)]

        mse_loss = mae_loss_1 + mae_loss_2
