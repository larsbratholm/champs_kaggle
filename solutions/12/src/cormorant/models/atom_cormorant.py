...
from cormorant.nn import OutputLinear, OutputPMLP, OutputMLP, OutputAtomMLP, GetScalars
...
class AtomCormorant(nn.Module):
    ...
    def __init__(self, num_cg_levels, maxl, max_sh, num_channels, num_species,
        ...
        super(AtomCormorant, self).__init__()
        ...
        if top == 'linear':
            ...
            self.top_func = OutputMLP(num_scalars, activation=activation, device=self.device, dtype=self.dtype)
        elif top == 'amlp':
            self.top_func = OutputAtomMLP(num_scalars, activation=activation, device=self.device, dtype=self.dtype)
        else:
