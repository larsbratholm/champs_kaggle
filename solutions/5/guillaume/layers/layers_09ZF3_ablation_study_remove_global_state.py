...
class MEGNetLayer(torch.nn.Module):
    def __init__(self, input_hidden, hidden):
        ...
        self.edge_mlp_for_edges_and_global = BasicBlock(2 * hidden + EDGE_TRIANGLE_SIZE, hidden)
        
        self.node_mlp = BasicBlock(2 * hidden, hidden)
        ...
        self.proj_current = nn.Linear(hidden, EDGE_TRIANGLE_SIZE)
        self.proj_bounded = nn.Linear(hidden, EDGE_TRIANGLE_SIZE)
        
        self.agg_edge_connectivity_1 = AttentionAggregation(3 * EDGE_TRIANGLE_SIZE, EDGE_TRIANGLE_SIZE)
        
        
    def edge_model(self, src, dest, edge_attr,
                   cycles_edge_index, cycles_id,
                   edges_connectivity_ids, edges_connectivity_features):
        ...
        #"""
        original_edge, neighbor_edge = edges_connectivity_ids
        ...
        original_embeddings = edge_attr[original_edge]
        neighbor_embeddings = edge_attr[neighbor_edge]
        edge_connectivity_repr = torch.cat([transformed_edges_features, self.proj_current(original_embeddings), self.proj_bounded(neighbor_embeddings)], dim = 1)
        out_edge_connectivity = self.agg_edge_connectivity_1(edge_connectivity_repr, original_edge, edge_attr.shape[0])
        #"""
        
        edge_and_global = torch.cat([edge_attr, out_edge_embeddings_cycle_aggregated, out_edge_connectivity], dim = 1)
    ...
    def node_model(self, x, row, col, edge_attr):
        ...
        out = torch.cat([mean_edge, x], dim=1)
    ...
    def forward(self, x, edge_index, edge_attr,
        ...
        x_input, edge_attr_input = x, edge_attr
        
        x = self.pre_node_mlp(x)
        edge_attr = self.pre_edge_mlp(edge_attr)
        
        # Edge model
        new_edge_attr = self.edge_model(x[row], x[col], edge_attr,
                                        cycles_edge_index, cycles_id,
                                        edges_connectivity_ids, edges_connectivity_features)
        ...
        # Node model
        new_x = self.node_model(x, row, col, double_new_edge_attr)
        new_x += x_input
...
class MEGNetList(torch.nn.Module):
    def __init__(self, count, nodes_embedding_in, edge_embedding_in, globals_embedding_in, nodes_in, edge_in, globals_in, 
        ...
        self.projs = nn.ModuleList([
            nn.Sequential(BasicBlock(3 * hidden, 1), nn.Linear(1, 1)) for _ in range(8)
        ])
    ...
    def forward(self,
        ...
        for i in range(len(self.op)):
            x, edge_attr = self.op[i](x, edge_index, edge_attr,
        ...
        predict = torch.cat([node0, node1, edge_attr],-1)
