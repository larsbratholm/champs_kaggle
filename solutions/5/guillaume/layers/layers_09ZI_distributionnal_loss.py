...
class MEGNetLayer(torch.nn.Module):
    def __init__(self, input_hidden, hidden):
        ...
        self.edge_mlp_for_edges_and_global = BasicBlock(4 * hidden, hidden)
        ...
        self.agg_edge_connectivity_1 = AttentionAggregation(3 * hidden, hidden)
    ...
    def edge_model(self, src, dest, edge_attr, u, batch,
        ...
        edge_connectivity_repr = torch.cat([transformed_edges_features, original_embeddings, neighbor_embeddings], dim = 1)
        out_edge_connectivity = self.agg_edge_connectivity_1(edge_connectivity_repr, original_edge, edge_attr.shape[0])
        #"""
        
        edge_and_global = torch.cat([edge_attr, u[batch], out_edge_embeddings_cycle_aggregated, out_edge_connectivity], dim = 1)
...
class MEGNetList(torch.nn.Module):
    def __init__(self, count, nodes_embedding_in, edge_embedding_in, globals_embedding_in, nodes_in, edge_in, globals_in, 
        ...
        self.projs = nn.ModuleList([
            BasicBlock(4 * hidden, BINS) for _ in range(8)
        ])
        """
        self.projs = BasicBlock(4 * hidden, BINS)
    ...
    def forward(self,
        ...
        """
        predicts = [proj(predict).unsqueeze(2) for proj in self.projs]
        predict = torch.cat(predicts, dim = 2)
        #predict = torch.gather(predict,2,target_types)
        predict = torch.gather(predict,2,target_types.unsqueeze(2).expand(-1, 260, -1)).squeeze(2)
        """
        predict = self.projs(predict)
